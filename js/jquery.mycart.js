...
function loadcart(){
  location.href="shipping_details.html"
}
...
function clearcart(){
  ...
  swal({
    title: "Are you sure?",
    text: "Once deleted, you can add it again!",
    icon: "warning",
    buttons: true,
    dangerMode: true,
  })
  .then((willDelete) => {
    if (willDelete) {
      ...
      swal("Poof! Your Cart Is Empty! Please add products to your cart", {
        icon: "success",
      }).then(function()
      {
  ...
  firebase.auth().onAuthStateChanged(function(user) {
      if (user) {
       loadcart();
        ...
        swal("Please Login First!", "We are redirecting you to login page. You need to Login or Sign in to continue with your purchase.", "error").then(function()
        {
  ...
  var loadMyCartEvent = function (targetSelector) {
    ...
    if (!$("#" + idCartModal).length) {
      $('body').append(
        ...
        '<button type="button" class="btn btn-default"  onclick="clearcart()" data-dismiss="modal">Clear Cart</button>' +
        //'<button type="button" class="btn btn-primary"  ' + classCheckoutCart + '">Checkout</button>' +
        '<button type="button"  class="btn btn-primary" onclick="checkIfLoggedIn()">Checkout</button>' +
        '</div>' +
        '</div>' +
        '</div>' +
        '</div>'
